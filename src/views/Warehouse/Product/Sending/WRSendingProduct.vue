  ...
  <section class="sendingProduct">
    <div class="breadcrumb p-0">
      ...
      <div class="breadcrumb__item">
        ...
        <router-link :to="{name: 'WRProduct', query: { name: 'Sending'}}" class="breadcrumb__link"><i class='bx bx-upload'></i></router-link>
      </div>
      <div class="breadcrumb__item">
        <i class="fa fa-angle-right"></i>
        <p>{{ $t('Материал') }}</p>
...
export default {
  ...
  methods: {
    listSendItems() {
      ...
      // this.info = [];
      this.infoSend = [];
      axios.get(`${this.api}/listSendItems/`, {
          headers: {
    ...
    openModalAdd() {
      ...
      this.$router.push({ name: 'WRSendingAddProduct' });
    },
    closeModalAdd() {
      this.modalAdd = false;
